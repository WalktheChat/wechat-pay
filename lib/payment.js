...
Payment.prototype.getBrandWCPayRequestParams = function(order, crossborder, callback) {
  var self = this;
  var default_params = {
    appId: this.appId,
  ...
  this.unifiedOrder(order, crossborder, function(err, data) {
    if (err) {
      return callback(err);
    }
...
Payment.prototype.unifiedOrder = function(params, crossborder, callback) {
  var requiredData = ['body', 'out_trade_no', 'total_fee', 'spbill_create_ip', 'trade_type'];
  if(crossborder){
    var requiredData = ['body', 'out_trade_no', 'total_fee', 'spbill_create_ip', 'trade_type', 'fee_type'];
  }
